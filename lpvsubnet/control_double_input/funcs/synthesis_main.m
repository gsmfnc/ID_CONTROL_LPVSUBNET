...
%% Non-normalized data
load('../../data/DBL_estim.mat')
train_y_unnorm = exp_y_est;
train_u_unnorm = [exp_u_est exp_p_est(:, 1)];
load('../../data/DBL_valid.mat')
test_y_unnorm = exp_y_val;
test_u_unnorm = [exp_u_val exp_p_val(:, 1)];

i2_norm_coeff = max(abs(train_u_unnorm(:, 1)));
i1_norm_coeff = max(abs(train_u_unnorm(:, 2)));
q1dot_norm_coeff = max(abs(train_u_unnorm(:, 3)));
q4dot_norm_coeff = max(abs(train_y_unnorm));

clear exp_u_est exp_u_val exp_p_est exp_p_val exp_y_est exp_y_val
